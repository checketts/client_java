...
public class TextParserTest {
    ...
    @Test
    public void it_shouldParseSimpleCounters(){
        String counterInput = "# TYPE a counter\n" +
                "# HELP a help\n" +
                "a 1";

        List<Collector.MetricFamilySamples> mfs = sut.parse(counterInput);

        assertThat(mfs).containsExactly(new CounterMetricFamily("a", "a help", 1));
    }

    @Test
    public void it_shouldParseSimpleGauges(){
        String counterInput = "# TYPE a gauge\n" +
                "# HELP a help\n" +
                "a 1";
    ...
    @Test
    public void it_shouldParseSimpleSummary(){
        String counterInput = "# TYPE a summary\n" +
                "# HELP a help\n" +
                "a_count 1\n" +
                "a_sum 2";
        ...
        assertThat(mfs).containsExactly(new SummaryMetricFamily("a", "a help", 1, 2));
    }

    @Test
    public void it_shouldParseSummaryWithQuantiles(){
        String counterInput = "# TYPE a summary\n" +
                "# HELP a help\n" +
                "a_count 1\n" +
                "a_sum 2\n" +
        ...
        SummaryMetricFamily summaryMetricFamily = new SummaryMetricFamily("a", "a help", Arrays.<String>asList(), Arrays.asList(0.5));
        summaryMetricFamily.addMetric(Arrays.<String>asList(), 1, 2, Arrays.asList(0.7));

        assertThat(mfs).containsExactly();
    }
...
//        self.assertEqual([metric_family], list(families))
    ...
    @Test //test timestamps discarded
    public void it_shouldDiscardTimestamps(){
        String metricsInput = "# TYPE a counter\n" +
                "# HELP a help\n" +
                "a{foo=\"bar\"} 1\\t000\n" +
                "# TYPE b counter\n" +
                "# HELP b help\n" +
        ...
        List<Collector.MetricFamilySamples> mfs = sut.parse(metricsInput);

        CounterMetricFamily a = new CounterMetricFamily("a", "help", Arrays.asList("foo"));
        a.addMetric(Arrays.asList("bar"), 1);
        CounterMetricFamily b = new CounterMetricFamily("b", "help", 2);

        assertThat(mfs).contains(a);
        assertThat(mfs).contains(b);
    }
